...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        theta = np.hstack([0, theta[1:]])
        J = 1/(2*num_examples) * np.dot(diff.T, diff) + reg/(2*num_examples) * np.dot(theta, theta.T)
    ...
    def grad_loss(self,*args):                                                                          
        ...
        m = X.shape[0]
        diff = np.dot(X, theta) - y
        grad = (1./m) * np.dot(np.transpose(diff), X) + reg/m * np.hstack([0, theta[1:]])
